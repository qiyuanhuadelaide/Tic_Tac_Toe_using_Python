...
class TicTacToeSim:
    ...
    def change_turn(self):
        ...
        if self.AI:  
            ...
            # computer will choose to random play or stop user to win based on that current board
            self.smart_move()  
            # self.print_board()
            return
        # if two human are playing, each user get one turn
        if self.turn == 1:
            self.turn = 2
        else:
            self.turn = 1
    ...
    def play_game(self):
        ...
        #     self.make_move(move=(1, 1), player=self.AI)  
        #     self.print_board()
        while True:  
            f = self.check_winner()  
            #  no winner so far
            if f == 0:  
                ...
                self.change_turn() 
            # there's no winner and all 9 spots are occupied, so it's a tie 
            elif f == -1:  
    ...
    def random_move(self):
        ...
        get the empty spot from current board
        """
        l = self.get_available_squares()  
        Length = len(l)  
    ...
    def winning_move(self, player):
        ...
        for i in range(0, 3):
            if self.board[i][0] == self.board[i][1] == player and 0 == self.board[i][2]:
                return i, 2
            elif self.board[i][0] == self.board[i][2] == player and 0 == self.board[i][1]:
                return i, 1
            elif self.board[i][1] == self.board[i][2] == player and self.board[i][0] == 0:
                return i, 0
        for i in range(0, 3):
            if self.board[0][i] == self.board[1][i] == player and 0 == self.board[2][i]:
                return 2, i
            elif self.board[0][i] == self.board[2][i] == player and 0 == self.board[1][i]:
                return 1, i
            elif self.board[1][i] == self.board[2][i] == player and self.board[0][i] == 0:
                return 0, i
        ...
        if self.board[0][2] == self.board[1][1] == player and 0 == self.board[2][0]:
            return 2, 0
        elif self.board[0][2] == self.board[2][0] == player and 0 == self.board[1][1]:
            return 1, 1
        elif self.board[1][1] == self.board[2][0] == player and self.board[0][2] == 0:
            return 0, 2
